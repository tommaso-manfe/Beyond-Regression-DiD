#EXPERIMENT 2A: NON-RANDOMIZED EXPERIMENT WITH X-SPECIFIC COMMON TREND, COMPOSITIONAL CHANGES AND HETEROGENEOUS EFFECTS
...
tripleIPWRA <- function(id, t, d, X, Y, method='logit'){
  ...
  if (method=='logit'){
    ...
    cov1 <- poly(as.matrix(data1[,9:ncol(data1)]), degree=2, raw=TRUE)
  ...
  if (method=='logit'){
    ...
    # LASSO linear regression
    # perform LASSO linear regression with 10-fold cross validation
    cov2 <- poly(as.matrix(data2[,9:ncol(data2)]), degree=2, raw=TRUE)
  ...
  if (method=='logit'){
    ...
    # perform LASSO linear regression with 10-fold cross validation
    cov3 <- poly(as.matrix(data3[,9:ncol(data3)]), degree=2, raw=TRUE)
...
for (i in 1:M){ #  M is the number of iterations
  ...
  index.att <- -10*z1+10*z2-10*z3-10*z4
  ...
  ti <- stats::plogis(- z1 + 0.5 * z2 - 0.25 * z3 - 0.1 * z4)
...
#EXPERIMENT 2B: NON-RANDOMIZED EXPERIMENT WITH X-SPECIFIC COMMON TREND, COMPOSITIONAL CHANGES AND HETEROGENEOUS EFFECTS
...
#EXPERIMENT 2C: NON-RANDOMIZED EXPERIMENT WITH X-SPECIFIC COMMON TREND, COMPOSITIONAL CHANGES AND HETEROGENEOUS EFFECTS
...
#EXPERIMENT 2D: NON-RANDOMIZED EXPERIMENT WITH X-SPECIFIC COMMON TREND, COMPOSITIONAL CHANGES AND HETEROGENEOUS EFFECTS
